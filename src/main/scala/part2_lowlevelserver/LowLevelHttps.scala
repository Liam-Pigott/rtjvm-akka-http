...
object HttpsContext {
  ...
  // init a trust manager
  val trustManagerFactor = TrustManagerFactory.getInstance("SunX509")
  trustManagerFactor.init(ks)

  // init SSL context
  val sslContext = SSLContext.getInstance("TLS")
  sslContext.init(keyManagerFactory.getKeyManagers, trustManagerFactor.getTrustManagers, new SecureRandom)
