...
class GameAreaMap extends Actor with ActorLogging {
  ...
  override def receive: Receive = {
    ...
    case GetPlayersByClass(characterClass) =>
      log.info(s"Getting players for class: $characterClass")
...
object MarshallingJSON extends App with PlayerJsonProtocol with SprayJsonSupport {
  ...
  val rtjvmGameMap = system.actorOf(Props[GameAreaMap], "rtjvmGameAreaMap")
  val playersList = List(
    Player("Martin_killer", "Warrior", 70),
  ...
  val rtjvmGameRouteSkel =
    pathPrefix("api" / "player") {
      get {
        ...
        post {
          //as[Player] under the hood: implicitly[FromRequestUnmarshaller[Player]]
